...
options = odeset('RelTol',1e-6,'AbsTol',1e-6,'MaxStep',0.01); % smoother PI simulation
...
title(['Joint 1 Angle vs Time | PI Controller Kp=' num2str(kp) ', Ki=' num2str(ki)]);
subplot(2,1,2);
plot(t, q2, 'b', 'LineWidth', 1.5); grid on;
xlabel('Time (s)'); ylabel('q2 (rad)');
title(['Joint 2 Angle vs Time | PI Controller Kp=' num2str(kp) ', Ki=' num2str(ki)]);
e1 = q_desired(1) - q1;
e2 = q_desired(2) - q2;
figure;
subplot(2,1,1);
